...
class LLMProvider(models.Model):
    ...
    def ollama_process_non_streaming_response(self, response):
        ...
        For models that support thinking (e.g., DeepSeek R1), the thinking
        content is wrapped in <think>...</think> tags and prepended to the
        actual response content.
        """
        content = response["message"].get("content") or ""
        thinking = response["message"].get("thinking")
        
        # If there's thinking content, prepend it with <think> tags
        if thinking:
    ...
    def ollama_process_streaming_response(self, response):
        ...
        # Track thinking content for models like DeepSeek R1
        ...
        try:
            for chunk in response:
                ...
                thinking_chunk = message.get("thinking")  # DeepSeek R1 thinking field
                ...
                # Handle thinking content from DeepSeek R1 models
                if thinking_chunk is not None and thinking_chunk != last_thinking:
                    if not thinking_started:
                        # Start the think block
                        thinking_started = True
                        yield {"content": "<think>"}
                    yield {"content": thinking_chunk}
                ...
                if content_chunk is not None and content_chunk != last_content:
                    # If we were in thinking mode and now getting content, close the think block
                    if thinking_started and not thinking_ended and accumulated_thinking:
                        thinking_ended = True
                        yield {"content": "</think>\n\n"}
        ...
        try:
            if model_id:
                ...
                if hasattr(models_response, "models"):
                    ...
                    raise ValueError(f"Unexpected Ollama API response format: {models_response}")

                for model in models:
                    yield self._ollama_parse_model(model)
        except Exception as e:
            _logger.exception("Error fetching Ollama models: %s", e)
            raise
    ...
    def ollama_format_messages(self, messages, system_prompt=None, model=None):
        ...
        Args:
            ...
            model: Optional model record (unused for Ollama but kept for API compatibility)
