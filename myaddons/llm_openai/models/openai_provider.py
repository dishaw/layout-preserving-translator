...
class LLMProvider(models.Model):
    ...
    def openai_get_client(self):
        """Get OpenAI client instance with optional proxy support"""
        http_client = None
        if self.proxy_url:
    ...
    def openai_models(self, model_id=None):
        ...
        try:
            ...
            raise UserError(
                ...
                "Please check the API key for provider '%s'." % self.name
            ) from exc
        except APIConnectionError as exc:
            raise UserError(
                "Could not connect to provider '%s'. Please check API Base, Proxy URL, "
                "DNS/network access from the Odoo container, and whether the proxy host exists. "
                "API Base: %s. Proxy URL: %s."
                % (self.name, self.api_base or "-", self.proxy_url or "-")
            ) from exc
        except OpenAIError as exc:
            raise UserError(
                "OpenAI-compatible provider '%s' returned an error: %s"
                % (self.name, exc)
            ) from exc
